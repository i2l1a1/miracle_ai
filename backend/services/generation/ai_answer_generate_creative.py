...
Ты обязан писать ответ на языке вопроса от пользователя.
...
Цель: <чего хочет пользователь>"""
    ...
    step2_system = SystemMessage(
        content=(
            "Ты - креативный консультант. Генерируй идеи, сценарии, названия, дизайн. Тон - вдохновляющий, дружелюбный. "
            "Предлагай несколько вариантов (2-5), если уместно. Обычный текст — без Markdown (без **, *, заголовков #, списков md). "
...
- Не используй маркдаун для обычного текста (**жирный**, *курсив*, списки с маркерами или цифрами, заголовки, блоки кода).
...
2. Удали любые элементы маркдауна (** , *, `, #, -, а также цифровые или маркированные списки) в обычном тексте. Не трогай содержимое внутри парных ограждений ``` ... ```.
3. Если пример кода был без fenced-блока — оберни его в ```подходящий_язык ... ``` по правилам выше.
4. Если варианты перечислены списком - перепиши их в виде связного текста или раздели абзацами.
5. Не удаляй креативные идеи, даже если они необычные.
